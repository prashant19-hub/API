...
//get user record from map // return all user data in map as response
func getUser(w http.ResponseWriter , r *http.Request){                                         //check if the request method is POST, if not return 405 Method Not Allowed
   if r.Method != "GET"{
	  w.WriteHeader(http.StatusMethodNotAllowed)
	  return
    }
